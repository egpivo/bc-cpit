"""
Run multiple Design 1 replicates and write summary CSV.

Usage (from cpit package root):
  python -m examples.simulation.run_design1 --n_reps 100 --base_seed 42
"""

import argparse
from pathlib import Path

from .design1 import PIT_SEED, run_design1_replicate
from .runner import ALPHAS, TAU_GRID, aggregate_replicates, aggregate_local_diagnostics


def main() -> None:
    parser = argparse.ArgumentParser(description="Run Design 1 replicates.")
    parser.add_argument("--n_reps", type=int, default=100)
    parser.add_argument("--base_seed", type=int, default=42)
    parser.add_argument(
        "--summary_path",
        type=str,
        default="results/simulation/design1/design1_rep_summary.csv",
    )
    ...
    progress_path = (
        ...
        else summary_path.parent / "design1_rep_progress.csv"
    )

    print(
        f"Design 1: {args.n_reps} replicates "
    ...
    for r in range(args.n_reps):
        ...
        res = run_design1_replicate(seed, pit_seed=PIT_SEED, alphas=ALPHAS, tau_grid=TAU_GRID, mode=args.mode)
