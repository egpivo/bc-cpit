"""
Run multiple Design 3 replicates and write summary CSV.

Usage (from cpit package root):
  python -m examples.simulation.run_design3 --n_reps 100 --base_seed 42
"""

import argparse
from pathlib import Path

from .design3 import PIT_SEED, run_design3_replicate
from .runner import ALPHAS, aggregate_replicates, aggregate_local_diagnostics


def main() -> None:
    parser = argparse.ArgumentParser(description="Run Design 3 replicates.")
    parser.add_argument("--n_reps", type=int, default=100)
    parser.add_argument("--base_seed", type=int, default=42)
    parser.add_argument(
        "--summary_path",
        type=str,
        default="results/simulation/design3/design3_rep_summary.csv",
    )
    ...
    progress_path = (
        ...
        else summary_path.parent / "design3_rep_progress.csv"
    )

    print(
        f"Design 3: {args.n_reps} replicates "
    ...
    for r in range(args.n_reps):
        ...
        res = run_design3_replicate(seed, pit_seed=PIT_SEED, alphas=ALPHAS, mode=args.mode)
